# Custom Toast Reference

This app uses a small in-app toast system with three states:

- `processing`
- `success`
- `error`

The styling is tuned to the app theme:

- Processing: light blue background, primary blue icon, blue progress bar
- Success: soft green background, green icon, green progress bar
- Error: soft red background, red icon, red progress bar

## Main Hook

Use `useToast()` from [`src/context/ToastContext.tsx`](./src/context/ToastContext.tsx).

### Available methods

- `showSuccessToast(title, description?)`
- `showErrorToast(title, description?)`
- `runActionToast(action, options)`
            ...
            <div className="border-b border-[#EEF3F8] bg-[#F8FBFF] px-4 py-3 dark:border-white/10 dark:bg-[#0F172A]/30">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-1 inline-flex items-center gap-1.5 rounded-full bg-[#E7F5FF] px-2 py-0.5 text-xs font-semibold tracking-[0.08em] text-[#286CFF] dark:bg-[#286CFF]/15 dark:text-[#B0DBFF]">
...
await runActionToast(
  ...
  {
    processingTitle: 'Saving record',
    processingDescription: 'Please wait while the record is being updated.',
...
If you want, I can also extract this into a standalone reusable component file like `src/components/shared/ActionToast.tsx` for easier copy-paste into other projects.
